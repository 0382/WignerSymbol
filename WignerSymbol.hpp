...
template <typename T, typename = typename std::enable_if_t<std::is_integral_v<T>>>
inline bool is_same_parity(T n1, T n2)
{
    return iseven(n1 ^ n2);
}
...
template <typename T, typename = typename std::enable_if_t<std::is_integral_v<T>>>
inline bool check_jm(T dj, T dm)
{
    return is_same_parity(dj, dm) && (std::abs(dm) <= dj);
}
...
class WignerSymbols
{
  public:
    ...
    {
        // initialize the data
        _binominal_data.resize(_binominal_data_size(_nmax));
        std::copy(std::begin(u64_binominal_data), std::end(u64_binominal_data), _binominal_data.begin());
    }
    double binominal(int n, int k) const
    {
        ...
        else
        {
            if (k > n / 2)
                k = n - k;
            return _binominal_data[_binominal_index(n, k)];
        }
    }
    double CG(int dj1, int dj2, int dj3, int dm1, int dm2, int dm3) const
    {
        ...
        int j2pm2 = (dj2 + dm2) / 2;
        double A =
            std::sqrt(binominal(dj1, jm2) * binominal(dj2, jm3) /
                      (binominal(J + 1, jm3) * binominal(dj1, j1mm1) * binominal(dj2, j2mm2) * binominal(dj3, j3mm3)));
        double B = 0;
        int low = std::max(0, std::max(j1mm1 - jm2, j2pm2 - jm1));
        int high = std::min(jm3, std::min(j1mm1, j2pm2));
        for (auto z = low; z <= high; ++z)
        {
            B = -B + binominal(jm3, z) * binominal(jm2, j1mm1 - z) * binominal(jm1, j2pm2 - z);
        }
    ...
    double f6j(int dj1, int dj2, int dj3, int dj4, int dj5, int dj6) const
    {
        ...
        double A = std::sqrt(binominal(j123 + 1, dj1 + 1) * binominal(dj1, jpm123) /
                             (binominal(j156 + 1, dj1 + 1) * binominal(dj1, jpm156) * binominal(j453 + 1, dj4 + 1) *
                              binominal(dj4, jpm453) * binominal(j426 + 1, dj4 + 1) * binominal(dj4, jpm426)));
        double B = 0;
        ...
        {
            B = -B + binominal(x + 1, j123 + 1) * binominal(jpm123, x - j453) * binominal(jpm132, x - j426) *
                         binominal(jpm231, x - j156);
        }
    ...
    double f9j(int dj1, int dj2, int dj3, int dj4, int dj5, int dj6, int dj7, int dj8, int dj9) const
    {
        ...
        double P0_nu = binominal(j123 + 1, dj1 + 1) * binominal(dj1, pm123) *
                       (binominal(j456 + 1, dj5 + 1) * binominal(dj5, pm456) * binominal(j789 + 1, dj9 + 1) *
                        binominal(dj9, pm798));
        double P0_de = binominal(j147 + 1, dj1 + 1) * binominal(dj1, (dj1 + dj4 - dj7) / 2) *
                       binominal(j258 + 1, dj5 + 1) * binominal(dj5, (dj2 + dj5 - dj8) / 2) *
                       binominal(j369 + 1, dj9 + 1) * binominal(dj9, (dj3 + dj9 - dj6) / 2);
        ...
        for (auto dt = dtl; dt <= dth; dt += 2)
        {
            ...
            double Pt_de = binominal(j19t + 1, dt + 1) * binominal(dt, (dj1 + dt - dj9) / 2) *
                           binominal(j26t + 1, dt + 1) * binominal(dt, (dj2 + dt - dj6) / 2) *
                           binominal(j48t + 1, dt + 1) * binominal(dt, (dj4 + dt - dj8) / 2);
            Pt_de *= (dt + 1) * (dt + 1);
            ...
            {
                At = -At + binominal(x + 1, j123 + 1) * binominal(pm123, x - j369) * binominal(pm132, x - j26t) *
                               binominal(pm231, x - j19t);
            }
            ...
            {
                Bt = -Bt + binominal(y + 1, j456 + 1) * binominal(pm456, y - j26t) * binominal(pm465, y - j258) *
                               binominal(pm564, y - j48t);
            }
            ...
            {
                Ct = -Ct + binominal(z + 1, j789 + 1) * binominal(pm789, z - j19t) * binominal(pm798, z - j48t) *
                               binominal(pm897, z - j147);
            }
    ...
    void reserve(int num, std::string type, int rank)
    {
        if (type == "Jmax")
        {
            switch (rank)
            {
                case 3: fill_binominal_data(3 * num + 1); break;
                case 6: fill_binominal_data(4 * num + 1); break;
                case 9: fill_binominal_data(5 * num + 1); break;
                default:
                {
        ...
        else if (type == "2bjmax")
        {
            switch (rank)
            {
                case 3: fill_binominal_data(2 * num + 1); break;
                case 6: fill_binominal_data(3 * num + 1); break;
                case 9: fill_binominal_data(4 * num + 1); break;
                default:
                {
                    std::cerr << "Error: rank must be 3, 6, or 9" << std::endl;
                    std::exit(-1);
  ...
  private:
    std::vector<double> _binominal_data;
    int _nmax;
    static std::size_t _binominal_data_size(int n)
    {
        std::size_t x = n / 2 + 1;
        return x * (x + isodd(n));
    }
    static std::size_t _binominal_index(int n, int k)
    {
        std::size_t x = n / 2 + 1;
        return x * (x - iseven(n)) + k;
    }
    void fill_binominal_data(int nmax)
    {
        if (nmax <= _nmax)
            return;
        std::vector<double> old_data = _binominal_data;
        _binominal_data.resize(_binominal_data_size(nmax));
        std::copy(std::begin(old_data), std::end(old_data), _binominal_data.begin());
        for (int n = _nmax + 1; n <= nmax; ++n)
        {
            for (int k = 0; k <= n / 2; ++k)
            {
                _binominal_data[_binominal_index(n, k)] = binominal(n - 1, k) + binominal(n - 1, k - 1);
            }
            ++_nmax;
        }
        _nmax = nmax;
